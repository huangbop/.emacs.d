...
(defconst json-mode-beautify-command-python2
  "python -c \"import sys,json,collections; data=json.loads(sys.stdin.read(),object_pairs_hook=collections.OrderedDict); print json.dumps(data,sort_keys=%s,indent=4,separators=(',',': ')).decode('unicode_escape').encode('utf8','replace')\"")
...
(defun json-mode-beautify (beg end &optional preserve-key-order)
  ...
  (shell-command-on-region (if mark-active beg (point-min))
                           ...
                           (concat (if (executable-find "env") "env " "")
                                   (format (if (executable-find "python")
